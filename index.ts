...
btnStart_take.addEventListener("click", () => {
  interval(500)
    ...
    .subscribe({
      next: e =>
        (result_take.textContent = result_take.textContent + " | " + e),
      complete: () =>
        (result_take.textContent = result_take.textContent + " | Completed")
    });
});
...
btnStart_takewhile.addEventListener("click", () => {
  interval(500)
    .pipe(takeWhile(e => e < 6))
    .subscribe({
      next: e =>
        (result_takewhile.textContent =
          result_takewhile.textContent + " | " + e),
      complete: () =>
        (result_takewhile.textContent =
          result_takewhile.textContent + " | Completed")
    });
});
...
btnStart_takeuntil.addEventListener("click", () => {
  interval(500)
    ...
    .subscribe(
      e =>
        (result_takeuntil.textContent =
          result_takeuntil.textContent + " | " + e)
    );
});
...
btnStart_scan.addEventListener("click", () => {
  interval(500)
    ...
    .subscribe(
      e => (result_scan.textContent = result_scan.textContent + " | " + e)
    );
  ...
  interval(500)
    ...
    .subscribe(
      e => (result_reduce.textContent = result_reduce.textContent + " | " + e)
    );
});
...
fromEvent(btnStart_custom, "click")
  .pipe(
...
//cutsom API call
const btnStop_apiCall = document.getElementById("btnStop_apiCall");
const result_apiCall = document.getElementById("result_apiCall");

// this is a mock for what an API could be, it returns an observable
function myApiCall() {
  return of("myValue");
}

const sub3 = timer(1000, 1000)
  .pipe(mergeMap(time => myApiCall()))
  .subscribe(e => (result_apiCall.textContent += e + " |"));

btnStop_apiCall.addEventListener("click", () => {
  sub3.unsubscribe();
});
